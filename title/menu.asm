...
SettablesTimer: .byte $00
SettablesRNG:   .byte $00
.popseg
...
UpdateValueTimer:
    ldx #2                 ; there are 2 total states
    jmp UpdateValueShared  ; update selected menu item
...
@update_value:
    ...
    ldy #$F                           ; otherwise wrap back around to F
    bne @store_value                  ; and store value
@increase:
    iny                               ; we're increment, so, increment Y
    cpy #$10                          ; check if we overflowed
...
; ===========================================================================
; Draws the two digit rng to screen
; ---------------------------------------------------------------------------
DrawValueRNG:
    clc                               ;
    lda VRAM_Buffer1_Offset           ; get current vram update offset
